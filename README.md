# FSRP Management — Official Intelligence Bot

**FSRP Management** is the elite intelligence and operations bot for Florida State Roleplay (FSRP). It is built on a **Zero-Persistence architecture** — Discord channels serve as the database. No MongoDB, no SQL, no Railway volumes required.
...
| `GUILD_ID` | Your FSRP Discord server ID |
...
- **Honor System** — Auto-promotes players to `@Played FSRP - 1 Hour`, `@Played FSRP - 2 Hour`.
...
| `/fsrp index` | Owner | Force re-index all rules channels |
| `/fsrp status` | Owner | View bot status and ERLC cache info |
| `@FSRP Management [question]` | Public | Ask anything — AI answers using rules + your game history |
...
Used for: welcome messages, application analysis, @FSRP Management Q&A, MDT dispatch, internal investigations.
